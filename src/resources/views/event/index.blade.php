...
<div class="card container text-center mb-5" id="mokumoku-lists">
    ...
    <div class="card-body">
        <div class="category text-left d-flex mb-1">
        ...
        <div class="content-wrapper d-flex justify-content-between">
            ...
            <div class="btn-filed">
                <a href="{{ route('event.show', ['id' => $event->event_id])}}" class="btn btn-primary">{{ '詳細' }}</a>
                <a href="{{ route('event.edit', ['id' => $event->event_id])}}" class="btn btn-info">{{ '編集' }}</a>
                <form action="{{ route('event.delete', ['id' => $event->event_id ]) }}" method="POST" class="d-inline">
                @csrf
    ...
    @endphp
    <div class="card-footer font-weight-bold bg-white">
